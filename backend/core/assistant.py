...
from core.translator import detect_language, translate_to_english, translate_from_english
...
def process_message_flow(
    ...
    try:
        ...
        messages = _build_messages(user_message, system_prompt, original_lang, needs_translation)

        ai_response = _get_ai_response(messages, temperature)

        final_response = _translate_response(ai_response, original_lang, needs_translation)
...
def _build_messages(
    ...
    translated_message = translate_to_english(user_message, source_lang) if needs_translation else user_message
...
def _translate_response(response: str, target_lang: str, needs_translation: bool) -> str:
